...
- **author**：name, name_original, aliases(JSON), bio, birth_date, death_date, ol_key …
- **work**：title, title_original, description, subjects(JSON), first_publish_year, ol_key …
- **edition**（主键 isbn_13）：work_id, title(这版书名), isbn_10, subtitle, **translators(JSON，版本级)**, publisher, publish_date, publish_year, cover_url, page_count, language, series, format, clc(中图法,留空待填), ol_key …
